<template>
  <div
    ...
    <form
      class="
        basis-1/2 min-w-0 p-[3vw] border-[2px] border-[#dedede] border-solid rounded-md bg-black/20 backdrop-blur-[12px]">
      ...
      <div class="flex flex-col gap-[1.5vw]">
        ...
        <div class="flex justify-between gap-[1vw]">
          <ButtonComponent icon="fi fi-rr-coffee-bean" class="flex-1" label="Adicionar Café" @click.prevent="AddCoffee(acervo,nome,produtor,origem,processo,pontoDeTorra)"/>
          <ButtonComponent icon="fi fi-rr-trash" class="flex-1" label="Limpar" @click.prevent="limpar"/>
        </div>
      </div>
    </form>
    <div class="basis-1/2 min-w-0 p-[3vw] border-[2px] border-[#dedede] border-solid rounded-md bg-black/20 backdrop-blur-[12px] flex flex-col justify-betwee">
  <div class="relative">
    <ul v-if="acervo.length !== 0" class=" flex flex-col gap-[0.7vw] max-h-[23vw] overflow-y-auto no-scrollbar">
      <li
        v-for="(cafe, index) in acervo"
        :key="index"
        class="text-white flex flex-col border-[2px] border-[#dedede] border-solid rounded-md py-[0.5vw] px-[1vw] text-[1.4vw]">
        <p>Nome: {{ cafe.nome !== '' && cafe.nome !== null ? cafe.nome : '--' }}</p>
        <p>Origem: {{ cafe.origem !== '' && cafe.origem !== null ? cafe.origem : '--' }}</p>
        <p>Produtor: {{ cafe.produtor !== '' && cafe.produtor !== null ? cafe.produtor : '--' }}</p>
        <p>Ponto de torra: {{ cafe.ponto_de_torra !== '' && cafe.ponto_de_torra !== null ? cafe.ponto_de_torra : '--' }}</p>
      </li>
    </ul>
...
import { AddCoffee,LimparForm } from './functions';
import { ref } from 'vue';
